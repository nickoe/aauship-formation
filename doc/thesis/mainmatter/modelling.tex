...
The linear model \eqref{eq:reducedmodel1} consists of the mass matrix $M_{RB}$, the damping matrix $D$ and the restoring force matrix $G$.
\begin{align}
M_{RB} \dot \nu_r + D\nu_r + G\eta = \tau_{RB} + \tau
...
\label{eq:testsystem}
\end{align}
The system is modelled as a 5 \ac{DOF} system and the nessesary coefficients are found in appendix \ref{app:damping}.

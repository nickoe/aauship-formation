...
\noindent To make a model for simulation model, it is needed to emulate the real
...
found in \ac{IMU}'s (accelerometer, gryommeter and magnetometer)
whilst the second does not need the magnetometer measurements.

In its simplest form an complementary filter is a filter, which takes
...
measures the angular rate, which again can integrated to give an
angular position. This position from the gyro, will drift over time,
but will be fairly accurate in a short lookahead time.

The complementary filter uses these properties fuse the
...
attitude from the gyro, hence the result is an estimate that is

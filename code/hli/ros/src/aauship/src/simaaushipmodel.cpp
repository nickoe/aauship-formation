...
int main(int argc, char **argv)
{
  ...
  gsl_matrix_set(Fxk,  2, 4, -0.000047368775740);
  gsl_matrix_set(Fxk,  3, 4, 0.004282051934167);
  gsl_matrix_set(Fxk,  4, 4, 0.845057894901144);
  gsl_matrix_set(Fxk,  5, 4, -0.001585132859674);
  gsl_matrix_set(Fxk,  6, 4, -0.006037628365521);
  gsl_matrix_set(Fxk,  7, 4, -0.001634208621250);
  gsl_matrix_set(Fxk,  8, 4, 0.158428534679433);
  gsl_matrix_set(Fxk,  9, 4, -5.869213055434965);
  gsl_matrix_set(Fxk, 10, 4, -0.054820991730417);
  gsl_matrix_set(Fxk, 11, 4, -0.230859896547226);
  ...
  gsl_matrix_set(Fxk,  2, 5, -0.004009144592212);
  gsl_matrix_set(Fxk,  3, 5, 0.000822953153928);
  gsl_matrix_set(Fxk,  4, 5, -0.029965504425292);
  gsl_matrix_set(Fxk,  5, 5, 0.865852803887239);
  gsl_matrix_set(Fxk,  6, 5, -0.001299812197086);
  gsl_matrix_set(Fxk,  7, 5, -0.147577962531828);
  gsl_matrix_set(Fxk,  8, 5, 0.027659274325152);
  gsl_matrix_set(Fxk,  9, 5, -1.036331359780794);
  gsl_matrix_set(Fxk, 10, 5, -4.948665811403822);
  gsl_matrix_set(Fxk, 11, 5, -0.045698363336979);
  ...
  gsl_matrix_set(Fxk,  4, 6, 0.0);
  gsl_matrix_set(Fxk,  5, 6, 0.0);
  ...
  gsl_matrix_set(Fxk,  0, 7, dt*cos(psi));
  gsl_matrix_set(Fxk,  1, 7, dt*sin(psi));
  gsl_matrix_set(Fxk,  2, 7,  0.049723406204869);
  gsl_matrix_set(Fxk,  3, 7,  0.000000533491234);
  gsl_matrix_set(Fxk,  4, 7, -0.000019426501116);
  gsl_matrix_set(Fxk,  5, 7, -0.000086975759482);
  gsl_matrix_set(Fxk,  6, 7, -0.000000842685134);
  gsl_matrix_set(Fxk,  7, 7,  0.988964802425052);
  gsl_matrix_set(Fxk,  8, 7,  0.000017884125782);
  gsl_matrix_set(Fxk,  9, 7, -0.000670202021289);
  gsl_matrix_set(Fxk, 10, 7, -0.003201606995903);
  gsl_matrix_set(Fxk, 11, 7, -0.000029556488868);
  gsl_matrix_set(Fxk, 12, 7,  0.988964802425052);
  gsl_matrix_set(Fxk, 13, 7,  0.000017884125782);
  gsl_matrix_set(Fxk, 14, 7, -0.000670202021289);
  gsl_matrix_set(Fxk, 15, 7, -0.003201606995903);
  gsl_matrix_set(Fxk, 16, 7, -0.000029556488868);
  // Derivatives for v(t+1)
  gsl_matrix_set(Fxk,  0, 8, dt*(-sin(psi)));
  gsl_matrix_set(Fxk,  1, 8, dt*cos(psi));
  gsl_matrix_set(Fxk,  2, 8,  0.000012127867596);
  gsl_matrix_set(Fxk,  3, 8,  0.045837548347937);
  gsl_matrix_set(Fxk,  4, 8,  0.039922222094321);
  gsl_matrix_set(Fxk,  5, 8,  0.000405862255594);
  gsl_matrix_set(Fxk,  6, 8, -0.000569417909974);
  gsl_matrix_set(Fxk,  7, 8,  0.000406627565023);
  gsl_matrix_set(Fxk,  8, 8,  0.840360099012158);
  gsl_matrix_set(Fxk,  9, 8,  1.477258087981388);
  gsl_matrix_set(Fxk, 10, 8,  0.013643189863155);
  gsl_matrix_set(Fxk, 11, 8, -0.024926387639480);
  gsl_matrix_set(Fxk, 12, 8,  0.000406627565023);
  gsl_matrix_set(Fxk, 13, 8,  0.840360099012158);
  gsl_matrix_set(Fxk, 14, 8,  1.477258087981388);
  gsl_matrix_set(Fxk, 15, 8,  0.013643189863155);
  gsl_matrix_set(Fxk, 16, 8, -0.024926387639480);
  // Derivatives for p(t+1)
  gsl_matrix_set(Fxk,  0, 9,                0.0);
  gsl_matrix_set(Fxk,  1, 9,                0.0);
  gsl_matrix_set(Fxk,  2, 9, -0.000001585666563);
  gsl_matrix_set(Fxk,  3, 9,  0.000141647982862);
  gsl_matrix_set(Fxk,  4, 9,  0.044921818677197);
  gsl_matrix_set(Fxk,  5, 9, -0.000053032470022);
  gsl_matrix_set(Fxk,  6, 9, -0.000196473915310);
  gsl_matrix_set(Fxk,  7, 9, -0.000073027846587);
  gsl_matrix_set(Fxk,  8, 9,  0.006788922250085);
  gsl_matrix_set(Fxk,  9, 9,  0.752899321745734);
  gsl_matrix_set(Fxk, 10, 9, -0.002445889774367);
  gsl_matrix_set(Fxk, 11, 9, -0.009625312696845);
  gsl_matrix_set(Fxk, 12, 9, -0.000073027846587);
  gsl_matrix_set(Fxk, 13, 9,  0.006788922250085);
  gsl_matrix_set(Fxk, 14, 9,  0.752899321745734);
  gsl_matrix_set(Fxk, 15, 9, -0.002445889774367);
  gsl_matrix_set(Fxk, 16, 9, -0.009625312696845);
  // Derivatives for q(t+1)
  gsl_matrix_set(Fxk,  0, 10,                0.0);
  gsl_matrix_set(Fxk,  1, 10,                0.0);
  gsl_matrix_set(Fxk,  2, 10, -0.000288466147122);
  gsl_matrix_set(Fxk,  3, 10,  0.000059754445484);
  gsl_matrix_set(Fxk,  4, 10, -0.002169255036631);
  gsl_matrix_set(Fxk,  5, 10,  0.040350145693658);
  gsl_matrix_set(Fxk,  6, 10, -0.000093936003883);
  gsl_matrix_set(Fxk,  7, 10, -0.012072492561263);
  gsl_matrix_set(Fxk,  8, 10,  0.002334197445616);
  gsl_matrix_set(Fxk,  9, 10, -0.086588459653789);
  gsl_matrix_set(Fxk, 10, 10,  0.595468162879000);
  gsl_matrix_set(Fxk, 11, 10, -0.003796674187051);
  gsl_matrix_set(Fxk, 12, 10, -0.012072492561263);
  gsl_matrix_set(Fxk, 13, 10,  0.002334197445616);
  gsl_matrix_set(Fxk, 14, 10, -0.086588459653789);
  gsl_matrix_set(Fxk, 15, 10,  0.595468162879000);
  gsl_matrix_set(Fxk, 16, 10, -0.003796674187051);
  // Derivatives for r(t+1)
  gsl_matrix_set(Fxk,  0, 11,                0.0);
  gsl_matrix_set(Fxk,  1, 11,                0.0);
  gsl_matrix_set(Fxk,  2, 11, -0.000000147096000);
  gsl_matrix_set(Fxk,  3, 11, -0.000004613815804);
  gsl_matrix_set(Fxk,  4, 11, -0.000454980362467);
  gsl_matrix_set(Fxk,  5, 11, -0.000004922258035);
  gsl_matrix_set(Fxk,  6, 11,  0.049680806806460);
  gsl_matrix_set(Fxk,  7, 11, -0.000005145503670);
  gsl_matrix_set(Fxk,  8, 11, -0.000202061143866);
  gsl_matrix_set(Fxk,  9, 11, -0.017394200825761);
  gsl_matrix_set(Fxk, 10, 11, -0.000172595493010);
  gsl_matrix_set(Fxk, 11, 11,  0.987291639691711);
  gsl_matrix_set(Fxk, 12, 11, -0.000005145503670);
  gsl_matrix_set(Fxk, 13, 11, -0.000202061143866);
  gsl_matrix_set(Fxk, 14, 11, -0.017394200825761);
  gsl_matrix_set(Fxk, 15, 11, -0.000172595493010);
  gsl_matrix_set(Fxk, 16, 11,  0.987291639691711);
  ...
  gsl_matrix_set(Fxk,  7, 13, 0.0);
  gsl_matrix_set(Fxk,  8, 13, 0.0);
  gsl_matrix_set(Fxk,  9, 13, 0.0);
  gsl_matrix_set(Fxk, 10, 13, 0.0);
  ...
  gsl_matrix_set(Fxk,  8, 14, 0.0);
  gsl_matrix_set(Fxk,  9, 14, 0.0);
  gsl_matrix_set(Fxk, 10, 14, 0.0);
  gsl_matrix_set(Fxk, 11, 14, 0.0);
  ...
  gsl_matrix_set(Gammaxk, 1, 0, -0.000000186535396);
  gsl_matrix_set(Gammaxk, 2, 0, 0.000006792482908);
  gsl_matrix_set(Gammaxk, 3, 0, 0.000030411104714);
  gsl_matrix_set(Gammaxk, 4, 0, 0.000000294645152);
  gsl_matrix_set(Gammaxk, 5, 0, 0.003858460690541);
  gsl_matrix_set(Gammaxk, 6, 0, -0.000006253190833);
  gsl_matrix_set(Gammaxk, 7, 0,  0.000234336371080);
  gsl_matrix_set(Gammaxk, 8, 0, 0.001119443005560);
  gsl_matrix_set(Gammaxk, 9, 0, 0.000010334436667);

  gsl_matrix_set(Gammaxk, 0, 1, -0.000000178190958);
  gsl_matrix_set(Gammaxk, 1, 1, 0.000109392085769);
  gsl_matrix_set(Gammaxk, 2, 1, -0.000587684722957);
  gsl_matrix_set(Gammaxk, 3, 1, -0.000005963511340);
  gsl_matrix_set(Gammaxk, 4, 1, 0.000008410239369);
  gsl_matrix_set(Gammaxk, 5, 1, -0.000005789352247);
  gsl_matrix_set(Gammaxk, 6, 1, 0.004218981105503);
  gsl_matrix_set(Gammaxk, 7, 1, -0.021191577426855);
  gsl_matrix_set(Gammaxk, 8, 1, -0.000194285141910);
  gsl_matrix_set(Gammaxk, 9, 1, 0.000361582197916);

  gsl_matrix_set(Gammaxk, 0, 2, 0.000006792585715);
  gsl_matrix_set(Gammaxk, 1, 2, -0.000614037503466);
  gsl_matrix_set(Gammaxk, 2, 2, 0.022218381481424);
  gsl_matrix_set(Gammaxk, 3, 2, 0.000227304815257);
  gsl_matrix_set(Gammaxk, 4, 2, 0.000865783578858);
  gsl_matrix_set(Gammaxk, 5, 2, 0.000234342179255);
  gsl_matrix_set(Gammaxk, 6, 2, -0.022718328364035);
  gsl_matrix_set(Gammaxk, 7, 2, 0.841633167293072);
  gsl_matrix_set(Gammaxk, 8, 2, 0.007861218270394);
  gsl_matrix_set(Gammaxk, 9, 2, 0.033104837751983);

  gsl_matrix_set(Gammaxk, 0, 3, 0.000030411104714);
  gsl_matrix_set(Gammaxk, 1, 3, -0.000006242457453);
  gsl_matrix_set(Gammaxk, 2, 3, 0.000227301378617);
  gsl_matrix_set(Gammaxk, 3, 3, 0.001017564803224);
  gsl_matrix_set(Gammaxk, 4, 3, 0.000009859640610);
  gsl_matrix_set(Gammaxk, 5, 3, 0.001119443005560);
  gsl_matrix_set(Gammaxk, 6, 3, -0.000209807620670);
  gsl_matrix_set(Gammaxk, 7, 3, 0.007861023910662);
  gsl_matrix_set(Gammaxk, 8, 3, 0.037537781619914);
  gsl_matrix_set(Gammaxk, 9, 3, 0.000346641953348);

  gsl_matrix_set(Gammaxk, 0, 4, 0.000000293445738);
  gsl_matrix_set(Gammaxk, 1, 4, 0.000008740819169);
  gsl_matrix_set(Gammaxk, 2, 4, 0.000862003431982);
  gsl_matrix_set(Gammaxk, 3, 4, 0.000009819546481);
  gsl_matrix_set(Gammaxk, 4, 4, 0.001169460635756);
  gsl_matrix_set(Gammaxk, 5, 4, 0.000010266674632);
  gsl_matrix_set(Gammaxk, 6, 4, 0.000382199901606);
  gsl_matrix_set(Gammaxk, 7, 4, 0.032882581027846);
  gsl_matrix_set(Gammaxk, 8, 4, 0.000344374423143);
  gsl_matrix_set(Gammaxk, 9, 4, 0.046617623605528);

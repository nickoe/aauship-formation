...
/*************************************************************************************/
// Evolution function
void f(gsl_vector * params, gsl_vector * xk_1, gsl_vector * xk)  // simaaushipmodel.m
{
...
void df(gsl_vector * params, gsl_vector * xk_1, gsl_matrix * Fxk)  // PHI fra matlab
{
    double n = gsl_vector_get(xk_1,0);
    double e = gsl_vector_get(xk_1,1);
    double x = gsl_vector_get(xk_1,2);
...
void h(gsl_vector * params, gsl_vector * xk , gsl_vector * yk) // vor lille h
{
...
void dh(gsl_vector * params, gsl_vector * xk , gsl_matrix * Hyk) // H_k i rapporten
{
...
{
    std::cout << "Start of kf-node callback" << std::endl;
  // @TODO Run KF
//	kf.KalmanFilterUpdate();
...
//    srand(time(NULL));
    ...
    // Initialize the parameter vector to zero values
    for(int i = 0 ; i < p.n ; i++)
        gsl_vector_set(s.xk,i,0);
...
*/
        ...
        printf("True state: %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e\nEstimated: %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e\n", 
            xi->data[0] ,
            xi->data[1] ,
            xi->data[2] ,
            xi->data[3] ,
...
//    std::cout << " You can plot them using e.g. gnuplot : " << std::endl;
    ...
    /**** Free the memory ****/
    /***********************************************/
    std::cout << "End of kf-node callback" << std::endl;
    ekf_free(p,s);
